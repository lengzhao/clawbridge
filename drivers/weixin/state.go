package weixin

import (
	"context"
	"crypto/sha256"
	"encoding/hex"
	"encoding/json"
	"fmt"
	"log/slog"
	"os"
	"path/filepath"
	"strings"
	"time"

	"github.com/lengzhao/clawbridge/client"
)

const (
	weixinDefaultCDNBaseURL    = "https://novac2c.cdn.weixin.qq.com/c2c"
	weixinConfigCacheTTL       = 24 * time.Hour
	weixinConfigRetryInitial   = 2 * time.Second
	weixinConfigRetryMax       = time.Hour
	weixinSessionPauseDuration = time.Hour
	weixinSessionExpiredCode   = -14
)

type typingTicketCacheEntry struct {
	ticket      string
	nextFetchAt time.Time
	retryDelay  time.Duration
}

type syncCursorFile struct {
	GetUpdatesBuf string `json:"get_updates_buf"`
}

type contextTokensFile struct {
	Tokens map[string]string `json:"tokens"`
}

func genWeixinAccountKey(baseURL, token string) string {
	token = strings.TrimSpace(token)
	if token == "" {
		return "default"
	}
	sum := sha256.Sum256([]byte(strings.TrimSpace(baseURL) + "|" + token))
	return hex.EncodeToString(sum[:8])
}

func buildWeixinSyncBufPath(stateRoot, accountKey string) string {
	return filepath.Join(stateRoot, "sync", accountKey+".json")
}

func buildWeixinContextTokensPath(stateRoot, accountKey string) string {
	return filepath.Join(stateRoot, "context-tokens", accountKey+".json")
}

func loadGetUpdatesBuf(path string) (string, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return "", nil
		}
		return "", err
	}
	var decoded syncCursorFile
	if err := json.Unmarshal(data, &decoded); err != nil {
		return "", err
	}
	return decoded.GetUpdatesBuf, nil
}

func saveGetUpdatesBuf(path, cursor string) error {
	data, err := json.Marshal(syncCursorFile{GetUpdatesBuf: cursor})
	if err != nil {
		return err
	}
	return atomicWriteFile(path, data, 0o600)
}

func loadContextTokens(path string) (map[string]string, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, err
	}
	var decoded contextTokensFile
	if err := json.Unmarshal(data, &decoded); err != nil {
		return nil, err
	}
	return decoded.Tokens, nil
}

func saveContextTokens(path string, tokens map[string]string) error {
	data, err := json.Marshal(contextTokensFile{Tokens: tokens})
	if err != nil {
		return err
	}
	return atomicWriteFile(path, data, 0o600)
}

func (d *driver) cdnBaseURL() string {
	if base := strings.TrimSpace(d.cdnBase); base != "" {
		return strings.TrimRight(base, "/")
	}
	return weixinDefaultCDNBaseURL
}

func isSessionExpiredStatus(ret, errcode int) bool {
	return ret == weixinSessionExpiredCode || errcode == weixinSessionExpiredCode
}

func (d *driver) pauseSession(operation string, ret, errcode int, errmsg string) time.Duration {
	d.pauseMu.Lock()
	defer d.pauseMu.Unlock()

	until := time.Now().Add(weixinSessionPauseDuration)
	if until.After(d.pauseUntil) {
		d.pauseUntil = until
	}

	remaining := time.Until(d.pauseUntil)
	slog.Error("weixin session expired; pausing channel",
		"client_id", d.id,
		"operation", operation,
		"ret", ret,
		"errcode", errcode,
		"errmsg", errmsg,
		"until", d.pauseUntil.Format(time.RFC3339),
		"minutes", int((remaining+time.Minute-1)/time.Minute),
	)
	return remaining
}

func (d *driver) remainingPause() time.Duration {
	d.pauseMu.Lock()
	defer d.pauseMu.Unlock()

	if d.pauseUntil.IsZero() {
		return 0
	}
	remaining := time.Until(d.pauseUntil)
	if remaining <= 0 {
		d.pauseUntil = time.Time{}
		return 0
	}
	return remaining
}

func (d *driver) waitWhileSessionPaused(ctx context.Context) error {
	remaining := d.remainingPause()
	if remaining <= 0 {
		return nil
	}
	timer := time.NewTimer(remaining)
	defer timer.Stop()
	select {
	case <-ctx.Done():
		return ctx.Err()
	case <-timer.C:
		return nil
	}
}

func (d *driver) ensureSessionActive() error {
	remaining := d.remainingPause()
	if remaining <= 0 {
		return nil
	}
	return fmt.Errorf(
		"weixin session paused (%d min remaining): %w",
		int((remaining+time.Minute-1)/time.Minute),
		client.ErrSendFailed,
	)
}

func (d *driver) getTypingTicket(ctx context.Context, userID string) (string, error) {
	now := time.Now()

	d.typingMu.Lock()
	entry, ok := d.typingCache[userID]
	if ok && now.Before(entry.nextFetchAt) {
		ticket := entry.ticket
		d.typingMu.Unlock()
		return ticket, nil
	}
	cachedTicket := entry.ticket
	retryDelay := entry.retryDelay
	d.typingMu.Unlock()

	contextToken := ""
	if v, ok := d.contextTokens.Load(userID); ok {
		contextToken, _ = v.(string)
	}

	resp, err := d.api.GetConfig(ctx, GetConfigReq{
		IlinkUserID:  userID,
		ContextToken: contextToken,
	})
	if err == nil && resp != nil && resp.Ret == 0 && resp.Errcode == 0 {
		ticket := strings.TrimSpace(resp.TypingTicket)
		d.typingMu.Lock()
		d.typingCache[userID] = typingTicketCacheEntry{
			ticket:      ticket,
			nextFetchAt: now.Add(weixinConfigCacheTTL),
			retryDelay:  weixinConfigRetryInitial,
		}
		d.typingMu.Unlock()
		return ticket, nil
	}

	if resp != nil && isSessionExpiredStatus(resp.Ret, resp.Errcode) {
		d.pauseSession("getconfig", resp.Ret, resp.Errcode, resp.Errmsg)
	}

	if retryDelay <= 0 {
		retryDelay = weixinConfigRetryInitial
	} else {
		retryDelay *= 2
		if retryDelay > weixinConfigRetryMax {
			retryDelay = weixinConfigRetryMax
		}
	}

	d.typingMu.Lock()
	d.typingCache[userID] = typingTicketCacheEntry{
		ticket:      cachedTicket,
		nextFetchAt: now.Add(retryDelay),
		retryDelay:  retryDelay,
	}
	d.typingMu.Unlock()

	if err != nil {
		return cachedTicket, err
	}
	if resp == nil {
		return cachedTicket, fmt.Errorf("getconfig returned nil response")
	}
	return cachedTicket, fmt.Errorf(
		"getconfig failed: ret=%d errcode=%d errmsg=%s",
		resp.Ret,
		resp.Errcode,
		resp.Errmsg,
	)
}
